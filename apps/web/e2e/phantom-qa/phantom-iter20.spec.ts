...
const SHOTS = resolve(HERE, "screenshots-iter20");
mkdirSync(SHOTS, { recursive: true });

const PRODUCTION = "https://use-settle.vercel.app";
const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

test.describe("iter20 verify Bug #51 inline error_message", () => {
  test.setTimeout(5 * 60 * 1000);

  test("/admin/health renders ↳ error_message under failed rows", async () => {
    ...
    await sleep(4_000);
    await page.screenshot({ path: resolve(SHOTS, "i20-01-admin-health-with-why.png"), fullPage: true });

    // Verify ↳ marker is in DOM
    const arrowCount = await page.evaluate(() => {
      const txt = document.body.innerText;
      return (txt.match(/↳/g) ?? []).length;
    });
    console.log(`↳ markers found: ${arrowCount}`);
    if (arrowCount === 0) {
      throw new Error("Bug #51 fix did not render any ↳ markers — failures may have rolled out of last-20 window");
    }
