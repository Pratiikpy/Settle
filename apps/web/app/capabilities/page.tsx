  ...
  return (
    <W6AppShell forceSurface="public">
      <div style={{ maxWidth: 880 }}>
        <div className="text-xs text-foreground/40">F9.2 · Public good</div>
        ...
        {focusedHash && (
          <div className="mt-6 rounded-xl border border-foreground/10 bg-foreground/[0.02] p-4 text-xs">
            <p className="text-foreground/50">Focused on hash</p>
            <code className="mt-1 block break-all font-mono text-foreground/85">
              {focusedHash}
            </code>
            <Link href="/capabilities" className="mt-2 inline-block text-foreground/60 hover:text-foreground">
              ← Clear filter
            </Link>
          </div>
        )}
        ...
        {!focusedHash && (
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            <input
              ...
              className="flex-1 rounded-lg border border-foreground/15 bg-transparent px-4 py-2 text-sm outline-none focus:border-accent"
            />
          ...
          <section className="mt-6 rounded-2xl border border-foreground/10 bg-white/[0.02] p-5">
            ...
            <div className="mt-4 grid gap-3 text-xs">
              <input
                ...
                className="rounded-lg border border-foreground/15 bg-transparent px-3 py-2 font-mono outline-none focus:border-accent"
              />
              <input
                value={formAlias}
                onChange={(e) => setFormAlias(e.target.value)}
                placeholder="Alias — e.g. Translate EN→FR"
                className="rounded-lg border border-foreground/15 bg-transparent px-3 py-2 outline-none focus:border-accent"
              />
              <textarea
                value={formDesc}
                ...
                className="rounded-lg border border-foreground/15 bg-transparent px-3 py-2 outline-none focus:border-accent"
              />
              <p className="mt-3 text-[11px] uppercase tracking-wide text-foreground/40">
          ...
          <ul className="grid gap-3">
            {entries.map((e) => (
              <li
                ...
                className="rounded-2xl border border-foreground/10 bg-white/[0.02] p-4"
              >
                ...
                {e.description && (
                  <p className="mt-1 text-xs text-foreground/65">{e.description}</p>
                )}
                <code className="mt-2 block break-all font-mono text-[10px] text-foreground/40">
                  {e.capability_hash}
                </code>
                {(e.spec_domain || e.spec_path) && (
                  <div className="mt-2 grid grid-cols-3 gap-2 text-[10px] text-foreground/50">
