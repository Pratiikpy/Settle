...
- Name: section-23b-A-extended.spec.ts >> §23b.A extended · consumer matrix >> 23b.A69 — /activity inbox surface
- Location: e2e\section-23b-A-extended.spec.ts:158:3

# Error details

```
Error: apiRequestContext._wrapApiCall: ENOENT: no such file or directory, open 'C:\Users\prate\Downloads\solana\settle-protocol\apps\web\test-results\.playwright-artifacts-171\traces\70cbc0b19fb07d6da69d-8d63f88b115b21184db0.trace'
```
...
```ts
  ...
  68  |   });
  69  | 
  70  |   // Receipt detail surfaces
  71  |   test("23b.A28 — receipt detail page has receipt id displayed", async ({ browser }) => {
  72  |     test.setTimeout(60_000);
  73  |     const ctx = await openPersonaContext(browser, ALICE_KEY);
  74  |     try {
  75  |       const page = await ctx.newPage();
  76  |       await connect(page);
  77  |       const r = await page.goto("/receipts/f6066dac-5602-4918-882a-02305aa60365");
  78  |       expect(r?.status()).toBe(200);
  79  |       const html = await page.content();
  80  |       // Receipt id segment shows somewhere on the page
  81  |       expect(html).toMatch(/f6066dac/);
  82  |     } finally {
  83  |       await ctx.close();
  84  |     }
  85  |   });
  86  | 
  87  |   // Groups vote
  88  |   test("23b.A49 — /groups create surface", async ({ browser }) => {
  89  |     test.setTimeout(60_000);
  90  |     const ctx = await openPersonaContext(browser, ALICE_KEY);
  91  |     try {
  92  |       const page = await ctx.newPage();
  93  |       await connect(page);
